...
export function Hero() {
  ...
  // Per client decision: suppress orphaned countdown label when showCountdown is false
    ...
    <header className="w-full bg-[var(--surface)] border-b border-[var(--border)]">
      ...
      <div className="max-w-[1120px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="inline-flex items-center gap-2 self-start sm:self-auto py-1.5 px-3 rounded bg-[var(--surface-2)] border border-[var(--border)] text-xs md:text-sm text-[var(--body)] font-medium">
            <span className="inline-block w-2 h-2 rounded-full bg-[var(--accent)] animate-pulse" aria-hidden="true" />
            <span>{urgencyHeaderText}</span>
          </div>
        </div>
      </div>
      ...
      <section className="max-w-[1120px] mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="max-w-[680px] text-left">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="inline-flex items-center px-2.5 py-1 rounded text-xs md:text-sm font-semibold tracking-wide bg-[var(--surface-2)] text-[var(--ink)] border border-[var(--border)]">
              {liveBadgeBlock.text}
            </span>
            <CohortDate />
          </div>
          ...
          <h1 className="text-[32px] md:text-[48px] font-semibold text-[var(--ink)] leading-[1.15] tracking-tight mb-6">
            {h1Block.text}
          </h1>

          {/* Subheading & Value Proposition */}
          <p className="text-lg md:text-xl font-medium text-[var(--ink)] mb-3 leading-snug">
            {subhead1Block.text}
          </p>
          <p className="text-base md:text-lg text-[var(--body)] leading-[1.6] mb-8">
            {subhead2Block.text}
          </p>
          ...
          <div className="flex flex-col items-start gap-3">
            ...
            <p className="text-xs md:text-sm text-[var(--body)] font-medium">
