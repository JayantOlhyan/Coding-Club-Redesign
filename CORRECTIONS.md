...
|---|---|---|---|---|
| **5** | `"Limited Seats. Registration Closing In"` | Top announcement pill | **Conditional Drop**: When `showCountdown` is false, Block 5 is not rendered at all to prevent an orphaned sentence fragment that stops mid-thought with no timer. When the client enables `showCountdown: true`, Block 5 returns verbatim followed by the active countdown timer. Zero string surgery or regex trimming is permitted. | Confirm cohort start date / countdown activation. |
